...
const coursesCtrl = {
	...
	createcoursereview: async (req, res) => {
		try {
			...
			if (course) {
				const alreadyreviewed = course.reviews.find((r) => r.user.toString() === user._id.toString());
				if (alreadyreviewed) {
					return res.json({
	...
	updateCourse: async (req, res) => {
		try {
			const { name, category, price, goals, image, Prerequisites, description, audience, subcategorys, content, shortdescription } = req.body;

			console.log("--------------req booody-------------", req.body.content[0].lectures);
			const course = await Courses.findById(req.params.id);
			if (course) {
	...
	getsubcategorys: async (req, res) => {
		try {
			...
			flat.forEach((i) => {
				if (!(subcategorys.includes(i) || subcategorys.includes(i.toLowerCase()))) subcategorys.push(i);
			});
